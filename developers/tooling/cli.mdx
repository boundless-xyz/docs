...
which will detail all the relevant options and mandatory flags:
...
Module Configuration:
  ...
  Or configure while executing commands:
    Example: boundless prover balance-collateral --prover-rpc-url <url> --prover-private-key <key>
```
...
```bash Terminal
RUST_LOG=info boundless requestor submit request.yaml
```
...
```bash Terminal
# [!code word:--wait]
RUST_LOG=info boundless requestor submit request.yaml --wait
```
...
```bash Terminal
# [!code word:--offchain]
RUST_LOG=info boundless requestor deposit 0.002 # Enough for the request above; deposit more to cover multiple requests.
RUST_LOG=info boundless requestor submit request.yaml --wait --offchain
```
