...
The SDK auto-detects the storage backend from configured environment variables. When `PINATA_JWT` is set, it uses Pinata to upload programs and inputs to IPFS:

```rust
use boundless_market::StorageUploaderConfig;

// StorageUploaderConfig is a clap Args struct, so it picks up env vars automatically.
// If using clap, flatten it into your CLI args:
...
let client = Client::builder()
  .with_rpc_url(args.rpc_url)
  .with_private_key(args.private_key)
  .with_uploader_config(&args.storage_config) // [!code hl] // [!code focus]
  .await?
  .build()
  .await?;
```
...
<Warning>
  By default, the SDK generates S3 presigned URLs that expire after 12 hours. If your request takes longer to fulfill, provers cannot download your program or inputs after expiry. For long-running requests, use IPFS storage, set `S3_PUBLIC_URL=true` to return public HTTPS URLs (requires a public bucket), or set `S3_PRESIGNED=false` to use direct S3 URLs with appropriate bucket policies.
</Warning>
...
You can also provide credentials directly via the `GCS_CREDENTIALS_JSON` environment variable for cases where you load credentials from a secrets manager without writing to disk.
...
By default, the GCS uploader returns `gs://` URLs. If your bucket is publicly readable and you want provers to download without GCS credentials, set `GCS_PUBLIC_URL=true`. After each upload, a HEAD request verifies the object is publicly accessible.

<Tip>
  For public buckets, use `GCS_PUBLIC_URL=true` so provers can download via standard HTTPS without needing GCS credentials.
</Tip>
