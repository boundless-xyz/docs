...
<Warning>
  By default, the SDK generates S3 presigned URLs that expire after 12 hours. If your request has an expiry period greater than 12 hours to be fulfilled, provers will not be able to download your program or inputs after the URL expires. For long-running requests, consider using IPFS storage or setting `S3_NO_PRESIGNED=1` to use direct S3 URLs with appropriate bucket policies.
</Warning>
...
Provers enforce default limits on file and journal sizes. Requests exceeding these limits will be ignored by most provers.

#### File Size Limits

Programs and input files must be under **50MB**. This limit applies to:
- Guest program ELF binaries
- Input files uploaded to storage providers

This prevents DoS attacks that would force provers to download excessively large files, wasting bandwidth and storage resources.

#### Journal Size Limits

Journals that need to be delivered on-chain must be under **10KB**. This prevents griefing attacks where malicious requestors force provers to post expensive amounts of calldata on-chain, costing the prover significant gas fees.

If your journal exceeds 10KB, consider using the `ClaimDigestMatch` predicate and designing your application to store journals off-chain. See [Journal Size Limits](/developers/tutorials/proof-types#journal-size-limits) for details.
