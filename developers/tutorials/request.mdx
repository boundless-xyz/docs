...
The recommended default is to attempt offchain submission first and automatically fall back to onchain submission if needed:

```rust
// Create a request using new_request
let request = client.new_request().with_program(ECHO_ELF).with_stdin(echo_message.as_bytes());

// Submit the request either offchain or onchain
let (request_id, expires_at) = client.submit(request).await?; // [!code focus]
```

#### Onchain

To submit a request explicitly onchain (without attempting an offchain submission), use:

```rust
// Create a request using new_request
let request = client.new_request().with_program(ECHO_ELF).with_stdin(echo_message.as_bytes());

// Submit the request explicitly onchain
let (request_id, expires_at) = client.submit_onchain(request).await?; // [!code focus]
```

#### Offchain

To submit a request explicitly offchain (without an onchain fallback), use:

```rust
// Create a request using new_request
let request = client.new_request().with_program(ECHO_ELF).with_stdin(echo_message.as_bytes());

// Submit the request explicitly offchain
