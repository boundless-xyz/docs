...
When submitting requests onchain, the Boundless Market SDK needs to fund the request with ETH to cover the `max_price` of the proof. The SDK provides several funding modes to control how this funding is handled, allowing you to optimize gas costs and manage your onchain balance efficiently.
...
# use boundless_market::{Client, client::FundingMode, storage::storage_provider_from_env};
# use url::Url;
# #[derive(Clone)]
# struct Args {
#     rpc_url: Url,
...
{/* <StripRustCodeComments> */}
```rust
# use boundless_market::client::FundingMode;
...
# use boundless_market::client::FundingMode;
# async fn configure_threshold() -> Result<FundingMode> {
...
# use alloy::primitives::utils::parse_ether;
# use boundless_market::client::FundingMode;
# async fn configure_min_max() -> Result<FundingMode> {
