...
Provers enforce default limits on file and journal sizes to protect against griefing attacks. Requests exceeding these limits will be ignored by most provers.

**Program and Input Files:** Must be under **50MB** each. This applies to guest program ELF binaries and any input files. This limit prevents DoS attacks that would force provers to download excessively large files.

**Journal Size:** Journals delivered on-chain must be under **10KB**. This prevents griefing attacks where malicious requestors force provers to post expensive amounts of calldata on-chain, costing the prover significant gas fees.
