...
The `requestor`, `prover`, and `rewards` modules each track their own *active network*. The `setup` wizard prompts you to pick a network and stores per-chain RPC URL and private key under `~/.boundless/secrets.toml`, keyed by chain. Subsequent commands automatically use the active network for that module.
...
Switching only changes which chain the module talks to — the per-chain credentials configured during `setup` stay associated with their respective chains, so you can flip between chains without re-entering them.
...
`deposit-to` credits the requestor balance of a different address, while the deposit transaction itself is paid by the caller. Useful when funding a customer or shared requestor account.

```bash Terminal
# Deposit 0.01 ETH to <recipient>'s requestor balance, paid by your wallet.
...
`deposit-collateral-to` credits the collateral balance of a different prover address, while the transaction itself is paid by the caller. Useful for shared treasuries, automated top-up services, or staking on behalf of a managed prover.

```bash Terminal
# Deposit 50 ZKC of collateral to <recipient>, paid by your wallet.
