...
<Info>
If you have a multi-GPU system with mixed GPU models, the same `SEGMENT_SIZE` is used across all of them, so it must be set to the lowest common denominator of the cards on the host. To find that ceiling, benchmark the smallest-VRAM model in isolation — ideally on a single-GPU host of that model, which matches the canonical Boundless setup.
</Info>
...
The `gpu_prove_agent` service auto-detects every GPU on the host and spawns one prove-agent process per GPU. To get a clean per-GPU benchmark, the simplest setup is to benchmark on a host that has only the GPU model you're testing — that way `nvidia-smi -L` reports exactly one card and the auto-detect entrypoint targets it directly.

<Steps>
  <Step title="Confirm the host exposes only the GPU you want to benchmark">
    ```bash Terminal
    nvidia-smi -L
    ```
    Should list exactly one GPU. If the host has multiple GPUs and you can't dedicate the box, run the benchmark on a separate single-GPU machine of the same model — that matches the canonical Boundless setup, which assumes the prover gets the whole host.
  </Step>
...
No compose changes are required — `gpu_prove_agent` auto-detects every GPU on the host and spawns one prove-agent process per card. To use both 16GB GPUs, just put them both on the prover host; the entrypoint will pick them up automatically.
