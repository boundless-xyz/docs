...
MinIO is used for object storage, including proof inputs and receipts. A lifecycle rule is automatically applied on startup to expire objects under the `inputs/` prefix, preventing unbounded disk growth. The TTL is controlled by the `INPUT_TTL_DAYS` environment variable (default: 1 day).
