...
The broker fails loudly on startup if a chain it's been asked to serve has no RPC URL configured.

<Note>
Free RPC tiers can be sufficient for low-throughput provers — `ChainMonitorV2` (the default since v1.5) reduced per-block RPC load substantially. Paid plans are still recommended for production to handle traffic spikes without hitting rate limits.
</Note>
