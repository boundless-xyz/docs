...
To run a prover on multiple chains simultaneously, use per-chain env vars instead of (or alongside) the singular `PROVER_RPC_URL`:
...
The broker only serves chains it discovers an RPC URL for; chains without one are silently skipped. After starting the stack, check `just prover logs` for one `Starting pipeline for chain` line per chain you expect.

<Note>
Free RPC tiers can be sufficient for low-throughput provers — `ChainMonitorV2` (the new default in v2.0) reduced per-block RPC load substantially. Paid plans are still recommended for production to handle traffic spikes without hitting rate limits.
</Note>
