...
import { RELEASE_TAG } from '/snippets/release-version.mdx';
...
```bash Terminal
cargo install --locked --git https://github.com/boundless-xyz/boundless bento-client --branch {RELEASE_TAG} --bin bento_cli
```
...
```bash
cargo install --locked --git https://github.com/boundless-xyz/boundless boundless-cli --branch {RELEASE_TAG} --bin boundless
```
