...
The setup wizard writes the same derived settings (currently `exec_agent` replica count and `SEGMENT_SIZE`) to both files, so you only need to edit the one matching the stack you actually run.
</Note>
...
The `gpu_prove_agent` service auto-detects all GPUs visible to the container and spawns one prove agent per GPU — no manual per-GPU configuration is needed for the common case. You can verify which GPUs Bento will use with:
...
All listed GPUs will be picked up automatically — no compose changes are required. If you want Bento to use only some of the GPUs on a host, configure that at the host level (e.g. dedicate the prover machine to Boundless, or use the NVIDIA driver / Docker daemon to control GPU visibility). The recommended Boundless setup is to give the prover a host with exactly the GPUs you want it to use.
