...
```bash
cargo install --locked --git https://github.com/boundless-xyz/boundless bento-client --branch release-1.4 --bin bento_cli
```
...
Once Bento is running successfully, it is time to configure the [broker](/provers/proving-stack#what-is-the-broker). The [Boundless CLI](/developers/tooling/cli) provides an interactive setup wizard to help configure the [broker.toml](/provers/broker#settings-in-brokertoml) and the compose files.

<Note>
The Boundless repo ships two compose files:

- [`prover-compose.yml`](https://github.com/boundless-xyz/boundless/blob/main/prover-compose.yml) — the default prover stack (Redis-only, uses the prebuilt `prover-agent` image). This is what `just bento` / `just prover` run unless `PROVER_STACK=legacy` is set.
...
The setup wizard writes the same wizard-derived settings (e.g. GPU agents, `SEGMENT_SIZE`) to both files, so you only need to edit the one matching the stack you actually run.
</Note>
...
```bash
cargo install --locked --git https://github.com/boundless-xyz/boundless boundless-cli --branch release-1.4 --bin boundless
```
...
Answer the questions as required, based on your setup and requirements. Once finished, `broker.toml`, `prover-compose.yml`, and `compose.yml` will be created/edited with the same wizard-derived settings.
...
The compose file defines all services within Bento. By default, `just bento` / `just prover` use [`prover-compose.yml`](https://github.com/boundless-xyz/boundless/blob/main/prover-compose.yml); the legacy [`compose.yml`](https://github.com/boundless-xyz/boundless/blob/main/compose.yml) is used when `PROVER_STACK=legacy` is set. The examples below show `prover-compose.yml` — the equivalent stanzas exist in `compose.yml`.

### Multi GPU

Under the `exec_agent` service in `prover-compose.yml`, the default configuration utilises a single GPU:

```yml prover-compose.yml
deploy:
  resources:
    reservations:
      devices:
...
We can see that GPU 1 is listed with the device ID of `1`. To add this GPU, uncomment `gpu_prove_agent1` from `prover-compose.yml`:
...
You can also configure the `SEGMENT_SIZE` in `prover-compose.yml` under the `exec_agent` service; it defaults to 21:
